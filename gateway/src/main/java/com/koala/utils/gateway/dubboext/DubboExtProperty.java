package com.koala.utils.gateway.dubboext;
...
import com.koala.utils.gateway.responseEntity.CreditNotification;
import com.koala.utils.gateway.responseEntity.MessageNotification;
...
public class DubboExtProperty {
    ...
    public static void addCreditsGain(CreditNotification creditInfo) {
        if (creditInfo != null) {
            ...
            if (tmp == null) {
                addNotifications(ConstField.CREDIT, JSON.toJSONString(creditInfo, Serializer.ApiSerializerFeature.SERIALIZER_FEATURES));
            } else {
                addNotifications(ConstField.CREDIT,
                        tmp + JSONOBJECT_SEPARATOR + JSON.toJSONString(creditInfo, Serializer.ApiSerializerFeature.SERIALIZER_FEATURES));
            }
        }
    }
    ...
    public static void addMessageInfo(MessageNotification msgInfo) {
        if (msgInfo != null) {
            ...
            if (tmp == null) {
                addNotifications(ConstField.MSG, JSON.toJSONString(msgInfo, Serializer.ApiSerializerFeature.SERIALIZER_FEATURES));
            } else {
                addNotifications(ConstField.MSG, tmp + JSONOBJECT_SEPARATOR + JSON.toJSONString(msgInfo, Serializer.ApiSerializerFeature.SERIALIZER_FEATURES));
            }
        }
    }
